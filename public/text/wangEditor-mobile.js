		...
		menus[menuId] = {
			...
			renderForm: function () {
				var fromHtml = [
					...
					'    method="post">',
					'    <input id="' + inputFileId + '" type="file"  name="pic">',
					'</form>'
				].join('');
			...
			bindEvent: function (editor) {
				...
				$inputFlie.on('change', function (e) {
					...
					reader.onload = function (e) {
						...
						formData.append('wangEditorH5File', convertBase64UrlToBlob(base64, fileType));
						var f_name = $inputFlie[0];
						formData.append('wangEditorH5File', f_name);
						xhr.send(formData);
						// alert(xhr)
					};
	...
	E.fn.bindTxtEvent = function () {
		...
		$txt.on('focus', function () {
			...
			if (/(iPhone|iPad|iPod|iOS)/i.test(navigator.userAgent)) {  //判断iPhone|iPad|iPod|iOS
				// window.webkit.messageHandlers.openNoBuy.openEditTool(true);
		...
		$txt.on('blur', function (e) {
			if (/(iPhone|iPad|iPod|iOS)/i.test(navigator.userAgent)) {  //判断iPhone|iPad|iPod|iOS
				// window.webkit.messageHandlers.openNoBuy.closeEditTool(true);
			} else if (/(Android)/i.test(navigator.userAgent)) {   //判断Android
				//alert(navigator.userAgent); 
				skill.closeEditTool();
